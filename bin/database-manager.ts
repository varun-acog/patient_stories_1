...
interface Options {
  ...
  analysisFile?: string;
  clear?: boolean;
  list?: string; // Table name to list (videos, transcripts, analysis)
  videoId?: string; // Filter for list or delete
  delete?: string; // Video ID to delete
  createVideo?: string; // JSON string for creating a video record
  createTranscript?: string; // JSON string for creating a transcript record
  createAnalysis?: string; // JSON string for creating an analysis record
}
...
async function main() {
  ...
  if (!options.metadataFile && !options.transcriptsFile && !options.analysisFile && !options.clear && 
  ...
  try {
    ...
    // Process files (existing Create functionality)
    ...
    if (options.transcriptsFile) {
      ...
      for (const { videoId, transcript, language } of transcripts) {
        await storeTranscript(videoId, transcript, language || "en");
